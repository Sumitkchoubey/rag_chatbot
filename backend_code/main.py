...
@app.post("/test-google-drive-url")
async def test_google_drive_url(request: TestURLRequest):
    """Test Google Drive URL conversion and access"""
    try:
        url = request.url
        original_url = url
        is_gdrive = URLProcessor.is_google_drive_url(url)
        converted_url = URLProcessor.convert_google_drive_url(url) if is_gdrive else url
        
        # Try to fetch just the headers
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
        }
        response = requests.head(converted_url, headers=headers, timeout=10, allow_redirects=True)
        
        return {
            "original_url": original_url,
            "is_google_drive": is_gdrive,
